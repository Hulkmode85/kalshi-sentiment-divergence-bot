...
try:
    if _brier_scorer_available:
        _brier_scorer.record(predicted_prob=locals().get("predicted_prob", locals().get("entry_price", 50)) / 100.0 if locals().get("predicted_prob", locals().get("entry_price", 50)) > 1 else locals().get("predicted_prob", 0.5), actual_outcome=1.0 if locals().get("won", locals().get("pnl", 0) > 0) else 0.0, asset=locals().get("asset", "sentiment-divergence"))
except Exception:
    pass
try:
    if _data_pipeline_available:
        _data_pipeline.record_snapshot({"bot": "sentiment-divergence", "event": "resolution", "pnl": locals().get("pnl", 0), "ts": time.time()})
except Exception:
    pass
...
try:
    if _pre_trade_validator_available:
        _ptv = validate_pre_trade({"ticker": locals().get("ticker", ""), "side": locals().get("side", ""), "bot": "sentiment-divergence"})
        ...
        while True:
            ...
            for topic_name, sentiment in sentiments.items():
                ...
                if not trade:
                    ...
                    evaluate_virtual_portfolios({"bot": "sentiment_divergence", "topic": topic_name})
                ...
                if regime == "CRASH":
                    log.warning("REGIME CRASH on kalshi_sentiment_divergence_bot — skipping trade")
                    shadow_log({"bot": "kalshi_sentiment_divergence_bot", "regime": regime}, taken=False, reason="crash regime")
                    evaluate_virtual_portfolios({"bot": "kalshi_sentiment_divergence_bot", "regime": regime})
                    continue
                ...
                evaluate_virtual_portfolios({"bot": "sentiment_divergence", "ticker": ticker, "side": trade["side"], "price": price, "edge": trade["edge"], "divergence": trade["divergence"]})
                # ── PRE-TRADE + REJECTION + DYNAMIC EDGE + SIZING (10-module integration) ──
                try:
                    if _pre_trade_validator_available:
                        _ptv = validate_pre_trade({"ticker": locals().get("ticker", ""), "side": locals().get("side", ""), "bot": "sentiment-divergence"})
